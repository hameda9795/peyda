...
export const CATEGORIES_CSV = `Hoofdcategorie (SEO),Subcategorieën (SEO),Voorbeelden (keywords),Aanbevolen URL-slug
...
export function getCategories(): BusinessCategory[] {
    ...
    for (const line of lines) {
        ...
        if (parts.length >= 4) {
            const name = parts[0];
            const cleanName = name.replace(' in Utrecht', '');
            categories.push({
                name: name,
                subcategories: parts[1].split(';').map(s => s.trim()),
                keywords: parts[2].split(',').map(s => s.trim()),
                slug: parts[3],
                thumbnail: getCategoryImage(parts[3])
            });
        }
    }
...
export function getCategoryBySlug(slug: string): BusinessCategory | undefined {
    ...
    const fullSlug = `/utrecht${normalizedSlug.startsWith('/utrecht') ? normalizedSlug.replace('/utrecht', '') : normalizedSlug}`;

    return categories.find(c => c.slug === fullSlug || c.slug === normalizedSlug);
}
